...
class TestSkolemization {
    ...
    private val formulas = mapOf(
            ...
            "\\ex A: R(A) & Q(sk1)" to "(R(sk-1) ∧ Q(sk1))",
            "!\\ex A : !(S(A) & !\\all B : (R(B) -> !R(A)))" to "!!(S(sk-1) ∧ !(∀B: (R(B) --> !R(sk-1))))",
            "!\\all A : (P(A) <-> \\ex C : (R(A) <-> !R(C)))" to "!(∀A: (P(A) <=> (R(A) <=> !R(sk-1(A)))))",
            "!\\ex A : R(A) -> !\\all B : !(R(B) | !R(B))" to "(!R(sk-1) --> !(∀B: !(R(B) ∨ !R(B))))"
    )
